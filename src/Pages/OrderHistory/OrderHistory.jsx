...
const OrderHistory = () => {
  ...
  const calculateTotalPrice = (products) => {
    return products.reduce(
      (total, product) => total + product.discountedPrice * product.itemCount,
      0
    );
  };
  ...
  const handleGiveRivew = (orderId, productId) => {
    localStorage.setItem("data", JSON.stringify(orderId));
    navigate(`/give-review/${productId}`);
  };
    ...
    return (
      <div className="w-full min-h-[80vh] flex items-center justify-center">
        <h1 className="text-center text-gray-400">
          ...
          <br />
          regular updates will be listed here
        </h1>
      </div>
    );
  }
        ...
        {sortedOrders.map((order) => {
          ...
          return (
            <div
              ...
              className="w-full p-6 rounded-md bg-white shadow-lg"
            >
              <div className="flex flex-col md:flex-row md:justify-between w-full gap-2">
                <div>
                  ...
                  <p className="text-sm text-gray-400">
                    Placed on {formatDate(order.orderCreationDate)}
                  </p>
                </div>
                {order.deliveredIn ? (
                  ...
                  <p className="text-yellow-400 text-sm md:w-64">
              ...
              <div className="w-full flex flex-col gap-3 pt-4">
                {order.products.map((item) => (
                  <div
                    ...
                    key={item.productId}
                    className="w-full flex justify-between gap-4 border-b p-6 rounded-md items-center hover:shadow-md"
                  >
                    ...
                    <div className="flex flex-col gap-1 items-center justify-center w-[20%]">
                      <p>{item.discountedPrice}tk</p>
                      <p className="text-xs ">Qty: {item.itemCount}</p>{" "}
                      {/* Added itemCount display */}
                      {order.orderStatus === "Delivered" && !item.reviews && (
                        <button
                          onClick={() => {
                            handleGiveRivew(order._id, item.productId);
                          }}
              ...
              <div className="flex justify-between   items-center">
                ...
                <p
                  className={`text-xs w-fit font-semibold text-white badge ${getBadgeClass(
                    order.orderStatus
                  )}`}
                >
                  {order.orderStatus}
