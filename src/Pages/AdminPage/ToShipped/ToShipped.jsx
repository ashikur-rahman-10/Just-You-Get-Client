...
const ToShipped = () => {
  ...
  const handleSort = () => {
    ...
    if (sortBy === "dateAsc") {
      sortedOrders.sort(
        (a, b) => new Date(a.orderCreationDate) - new Date(b.orderCreationDate)
      );
    } else if (sortBy === "dateDesc") {
      sortedOrders.sort(
        (a, b) => new Date(b.orderCreationDate) - new Date(a.orderCreationDate)
      );
  ...
  const handleDelivared = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure
          .patch(`orders/setDelivered/${id}`, update)
          .then((response) => {
  ...
  return (
    <div className="py-12 lg:py-0 bg-gray-50 px-4 min-h-screen">
      <h1 className="text-center text-2xl text-gray-400 font-semibold pt-10 font-mono">
        On Shipment: {orders?.length} Orders
      </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mx-auto w-fit text-xs py-10">
        {handleSort().map((order) => (
          <div
            key={order._id}
            className="relative w-[360px] md:w-[300px]  h-52 bg-white rounded-lg p-4 shadow-sm shadow-yellow-400 border-r-4  border-b-4 border-yellow-400"
          >
            <div className="pb-2">
              <p className=" text-gray-400">
                Customer:{" "}
                <span className="text-black"> {order?.client.name}</span>
              </p>
              <p className="text-gray-400">
                TransactionID:{" "}
                <Link
                  to={`/dashboard/order/${order?.transactionId}`}
              ...
              <div className="flex justify-between">
                ...
                <button
                  ...
                  className="bg-yellow-400 px-2 py-1 rounded-md text-white uppercase font-medium hover:bg-yellow-600 flex items-center gap-1"
                >
            ...
            <div className="bg-white">
              <div className="absolute right-4 bottom-4 flex">
                <Link
                  to={`/dashboard/order-details/${order.transactionId}`}
                  className="bg-blue-400 px-2 py-1 rounded-md text-white uppercase font-medium hover:bg-blue-600 flex items-center gap-1 text-md"
                >
                  Details <RiNewspaperLine />{" "}
                </Link>
              </div>
