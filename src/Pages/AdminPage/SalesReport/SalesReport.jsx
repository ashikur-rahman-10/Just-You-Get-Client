    ...
    if (startDate && endDate) {
      try {
        ...
        const response = await fetch(
          `https://just-you-get.vercel.app/sales-report?startDate=${startDate}&endDate=${adjustedEndDateString}`
        );
        if (!response.ok) {
          ...
          <table className="min-w-full bg-white border border-gray-300 rounded-md">
            ...
            <tbody>
              {salesData.salesItem.map((sale, index) => (
                <tr key={index}>
                  <td className="py-2 px-4 border-b border-gray-300 text-xs">
                    {sale.transactionId}
                  </td>
                  <td className="py-2 px-4 border-b border-gray-300 text-xs">
                    {sale.client.name}
                  ...
                  <td className="py-2 px-4 border-b border-gray-300 text-xs">
                    {new Date(sale.orderCreationDate).toLocaleDateString()}
                  </td>
                  <td className="py-2 px-4 border-b border-gray-300 text-xs">
                    {new Date(sale.deliveredIn).toLocaleDateString()}
                  </td>
                  <td className="py-2 px-4 border-b border-gray-300 text-xs">
                    {sale.totalPrice}
                  </td>
                      ...
                      {sale.products.map((product, productIndex) => (
                        ...
                        >
                          <img
                            ...
                            className="w-12 h-14 object-cover hidden md:block"
                          />
